...
export const addNodeReducer = (state: WritableDraft<EditorState>, action: PayloadAction<AnyNodeItem>) => {
...
export const updateNodeReducer = (state: WritableDraft<EditorState>, action: PayloadAction<AnyNodeItem>) => {
...
export const removeNodeReducer = (state: WritableDraft<EditorState>, action: PayloadAction<string>) => {
    ...
    state.presentation.nodes.allIds = state.presentation.nodes.allIds.filter((id) => id !== nodeId);

    // Убираем ссылку на node из слоя
    nodeParentLayer.nodeIds = nodeParentLayer.nodeIds.filter((id) => id !== nodeId);
...
export const selectNodeReducer = (state: WritableDraft<EditorState>, action: PayloadAction<string | null>) => {
    const nodeId = action.payload;
